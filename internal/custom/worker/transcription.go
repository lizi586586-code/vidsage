...
func (h *TranscriptionHandler) Run(ctx context.Context, job *model.VideoProcessingJob, video *model.Video) error {
	...
	if job.ExternalTaskID == "" {
		...
		validationURL := h.internalSourceURL(sourceURL)
		if err := h.Tongyi.ValidateSourceFile(ctx, validationURL); err != nil {
...
func (h *TranscriptionHandler) internalSourceURL(sourceURL string) string {
	return rewriteFrontendFileURL(sourceURL, h.InternalFrontendBaseURL)
}
