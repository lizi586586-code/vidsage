    ...
    <div class="node-panel-layer" @keydown.esc="emit('close')">
      ...
      <aside class="node-panel" role="dialog" aria-modal="true" aria-labelledby="node-panel-title" tabindex="-1">
        ...
        <article v-if="detailLoaded && detail" class="node-panel__article">
          ...
          <section v-if="relationGroups.length" class="node-panel__section node-panel__relations">
            <h3>知识关系 <small>{{ relationCount }}</small></h3>
            <div v-for="group in relationGroups" :key="group.key" class="node-panel__relation-group">
              <h4>{{ group.label }} <small>{{ group.links.length }}</small></h4>
              <ul class="node-panel__links">
                <li v-for="link in group.links" :key="link.key">
                  <button v-if="link.targetPageId" type="button" :aria-label="`在图谱中查看${link.title}`" @click="selectGraphNode(link)">「{{ link.title }}」</button>
                  <span v-else>「{{ link.title }}」</span>
                  <small v-if="link.type">{{ link.type }}<template v-if="link.confidence"> · 可信度 {{ Math.round(link.confidence * 100) }}%</template></small>
                </li>
              </ul>
            </div>
          </section>
...
import { getGraphRelationTypeLabel } from './knowledgeTypeStyles'
...
type RelationLink = GraphNodeTarget & { key: string; title: string; type?: string; confidence?: number; knowledgeType: string }
const formalLinks = computed<RelationLink[]>(() => props.relatedEdges.map((edge, index) => {
  ...
  return { key: edge.id || `${targetId}-${index}`, title, targetPageId, targetSlug: endpointSlug || target?.knowledge_detail?.slug, type: getGraphRelationTypeLabel(edge.type), confidence: edge.confidence, knowledgeType: knowledgeTypeFromNode(target) }
}))
const readingLinks = computed<RelationLink[]>(() => validReading.value.filter(item => item.target_title).map(item => {
  const target = props.relatedNodes.find(node => node.id === item.target)
  return { key: item.id, title: item.target_title || '', targetPageId: pageIdFromNodeId(item.target), targetSlug: item.target_slug, knowledgeType: knowledgeTypeFromNode(target) }
}))
const relationGroups = computed(() => {
  const groups = [
    { key: 'entity', label: '关联实体' },
    { key: 'concept', label: '关联概念' },
    { key: 'methodology', label: '关联方法论' },
    { key: 'case', label: '关联案例' },
    { key: 'insight', label: '关联洞察' },
  ]
  const links = [...formalLinks.value, ...readingLinks.value]
  return groups.map(group => ({ ...group, links: links.filter(link => link.knowledgeType === group.key) })).filter(group => group.links.length)
})
const relationCount = computed(() => relationGroups.value.reduce((total, group) => total + group.links.length, 0))
function knowledgeTypeFromNode(node?: GraphNode): string {
  const value = node?.knowledge_type || node?.knowledge_detail?.knowledge_type
  if (value && ['entity', 'concept', 'methodology', 'case', 'insight'].includes(value)) return value
  const attribute = node?.attributes?.[0] || node?.type || ''
  if (['人物', '机构', '产品', '技术', '行业', '地点', '实体'].includes(attribute)) return 'entity'
  return ({ 概念: 'concept', 方法论: 'methodology', 案例: 'case', 洞察: 'insight' } as Record<string, string>)[attribute] || ''
}
...
.node-panel__relation-group { display: grid; grid-template-columns: max-content minmax(0, 1fr); align-items: start; gap: 8px; min-width: 0; }
.node-panel__relation-group + .node-panel__relation-group { margin-top: 20px; }
.node-panel__relation-group h4 { margin: 0; color: var(--td-text-color-secondary); font-size: var(--td-font-size-body-medium); font-weight: 600; line-height: var(--td-line-height-body-medium); white-space: nowrap; }
...
.node-panel__relation-group .node-panel__links button, .node-panel__relation-group .node-panel__links li > span { display: block; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
